...
import com.ycx.manager.ClassUtil.DateFormat1;
import com.ycx.manager.bean.LoginUser;
import com.ycx.manager.bean.RegisterUser;
import com.ycx.manager.dao.AccountDao;
...
public class AccountServiceImpl implements AccountService {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ycx.manager.service.AccountService#saveUser(com.ycx.manager.bean.
	 * RegisterUser)
	 */
	@Override
	...
	public int addBackendUser(LoginUser loginUser) {
		loginUser.setMakeTime(new Date());
		int result = 0;
		result = accountDao.addBackendUser(loginUser);
		if (result == 1) {
			String userId = loginUser.getUserId();
			System.out.println(userId);
			loginUser.setUserId(String.valueOf(userId));
